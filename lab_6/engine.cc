...
/*
Uses the findAtIndex() helper (provided) to find the first match 
of the regular expression regex in the string s, 
and returns a Range object indicating where the match occurred
*/
...
bool match(vector<RegexOperator *> regex, const string &s){
    ...
    if (r.start == 0 && r.end == (int)s.length()){
        return true;
    }
    return false;
}
